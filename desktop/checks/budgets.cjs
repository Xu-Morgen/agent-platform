...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await run("const local=document.querySelector('[data-scope=\"packages.echo\"][data-budget-key=loopLimit]');local.value=3;local.dispatchEvent(new Event('change'));const strict=document.querySelector('[data-budget-key=strictTokenLimit]');strict.checked=false;strict.dispatchEvent(new Event('change'));document.querySelector('#service-form').requestSubmit()");
    await wait("document.querySelector('#service-result').textContent.includes('已保存版本 1.1')");
    await run('editService()');
    assert.equal(await run("document.querySelector('[data-scope=\"packages.echo\"][data-budget-key=loopLimit]').value"), '3');
    assert.equal(await run("document.querySelector('[data-budget-key=strictTokenLimit]').checked"), false);
    await run("showUsage({loops:{global:2,bindings:{echo:2}},tokens:{global:{inputTokens:3,outputTokens:2,totalTokens:5,reservedTokens:0,quality:'estimated',sources:['synthetic']},bindings:{echo:{inputTokens:3,outputTokens:2,totalTokens:5,reservedTokens:0,quality:'estimated',sources:['synthetic']}}}})");
    assert.match(await run("document.querySelector('#task-usage').textContent"), /任务总量.*估算.*synthetic/);
    assert.match(await run("document.querySelector('#task-usage').textContent"), /包 echo/);
    console.log('I4-T06 PASS: budget form creates version, reopened values match; synthetic estimated usage rendered locally/globally');
