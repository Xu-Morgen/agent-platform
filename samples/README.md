...
当前执行协议为 flow-6，块和最小包为 3.0.0，完整包为 4.0.0。主数据与参考由平台统一封装为 NodeInput；模块目录的 primaryContract 是可选服务入口，inputContract 是完整节点入口。完整包演示固定单参考，完整独立契约演示 ReviewInput；不增加第三套示例。旧快照可查历史，升级资源后须重新保存实例。

## 质量流程与模板版本

当前通用模板使用兼容 flow-6 的 `NodeInput` 资源，最小包与块版本为 3.0.0，完整包为 4.0.0；文档出题业务的版本独立管理，出题包现为 6.0.0，不意味着所有通用模板都必须同号升级。不要只改版本号而不检查源码内容和快照冲突。

导入新的包或块版本后，同类型、同声明 ID 的较低版本自动归档，补导入旧版也默认归档；可手动恢复，已有实例继续引用原资源。独立契约仍手动归档，详见 [使用说明](USAGE.md#5-保存版本与外部调用)。
...
数组遍历与枚举分支的页面操作、教学源码及升级边界见[控制流手册](../docs/control-flow.md)。
