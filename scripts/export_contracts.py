...
from agent_platform.contracts.files import FileReference

schemas = {'control': CONTROL.json_schema(by_alias=True), 'health': export_schema(HealthResponse), 'error': export_schema(ErrorResponse)}
schemas.update({'block-metadata': export_schema(BlockMetadata), 'file-reference': export_schema(FileReference)})
