...
[complete.py](complete.py)（`3.0.0`）在一个块内展示 API 能力、响应契约、嵌套输入、默认值、枚举、自定义字段校验、可选任务附件、进度上报、显式业务错误与输出统计。执行顺序为检查附件 → 查询 API → 严格验证响应 → 整理正文并检查 requireContent → 添加前缀 → 构造输出。源码包含分区中文讲解；模型文件接入为注释示范，默认不下载或运行模型。
