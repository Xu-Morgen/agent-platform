...
弹窗同时提供可复制的 Mermaid `stateDiagram-v2` 代码、`.mmd` 下载与 SVG 图片下载。代码和 SVG 由同一控制流模型生成，SVG 使用本地简易排版，无需联网、安装依赖或调用模型。长标签可在代码或 SVG 节点提示中查看完整内容。

这是帮助理解执行顺序的只读视图，不是运行时 LangGraph 内部节点图，也不提供导入执行。图中省略参考数据连线、契约重试、预算和取消路径；子服务保留固定实例标识，不展开内部流程。草稿可预览，但不代表通过契约校验。旧协议仍通过原始快照查看，不自动转换。
