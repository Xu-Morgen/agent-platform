...
@block(id='rag-question-wrap-judgment', version='4.0.0', name='封装判断题', description='主数据 Judgment；参考 tuple[()]；输出 QuestionEnvelope。')
def run(value: NodeInput[Judgment, tuple[()]]) -> QuestionEnvelope:
    try:
        return QuestionEnvelope(question=value.primary)
