...
6. 读取块修复版为 `1.0.1`；后续更新源码必须递增 `@block` 的 version，再重新加载资源，并在服务页为已有服务保存新实例版本，使后续任务使用新代码；仅重启应用不会替换已有实例快照。每个任务固定实例版本和文件副本。移动原文件不影响任务；持久化模式保留跨重启历史，`--memory` 模式仅供会话验证。
