...
def invoke(args):
    connection = {'connectionId':'model', 'kind':'model', 'baseUrl':args.model_url,
                  'model':args.model, 'timeoutSeconds':args.timeout}
    ...
    with httpx.Client(base_url=args.platform_url, timeout=30.0, trust_env=False) as client:
        ...
        record = {'status':run['status'], 'runId':run['runId'], 'serviceId':service['serviceId'],
                  'model':args.model, 'adapter':'ollama-chat', 'strictTokenLimit':not args.non_strict,
...
def main():
    ...
    parser.add_argument('--non-strict', action='store_true', help='显式使用非严格 token 模式；Ollama 当前必需')
