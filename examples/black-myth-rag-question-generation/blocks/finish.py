...
@block(id='rag-question-finish', version='4.0.0', name='生成题目最终业务结论', description='主数据 State；参考 tuple[()]；输出 ValidatedResult。')
def run(value: NodeInput[State, tuple[()]]) -> ValidatedResult:
    try:
        ...
        if review.verdict == 'pass':
            ...
            result = InsufficientResult(status='insufficient_source', requested_items=state.planned.normalized.items, reason=review.rationale,
