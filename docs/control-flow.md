...
图中展示服务输入输出和步骤契约的顶层字段摘要（`?` 表示字段可选），引用按固定资源 ID、子服务实例 ID 解析，不取同名资源最新版。条件与路由展示返回值及固定 Python 入口函数原文；入口调用的辅助函数不展开，不将任意 Python 推断为等价自然语言条件。布尔出口、枚举字符串（包括字符串 `"true"`）、循环上限、分支返回来源和循环携带值分别展示。资源目录的只读 `entrySource` 字段来自已保存源码。缺少资源或源码时明确显示不可用。
