...
@block(id='rag-question-insufficient', version='3.0.0', name='输出本次检索不足结论', description='主数据 Planned；参考 tuple[()]；输出 ValidatedResult。')
def run(value: NodeInput[Planned, tuple[()]]) -> ValidatedResult:
    try:
        ...
        return ValidatedResult(result=InsufficientResult(status='insufficient_source', requested_items=planned.normalized.items, reason=decision.reason,
