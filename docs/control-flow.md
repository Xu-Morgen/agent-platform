...
服务配置页点击「查看流程图」，预览当前编辑内容；服务版本记录中的同名按钮读取该版本固定快照，仅支持 `flow-6`。支持顺序、if/else、switch、repeat、while、foreach 和固定子服务节点，包含分支汇合、循环回边与循环上限错误出口。

弹窗同时提供可复制的 Mermaid `stateDiagram-v2` 代码、`.mmd` 下载与 SVG 图片下载。代码和 SVG 由同一控制流模型生成，SVG 使用本地分列排版，无需联网、安装依赖或调用模型。分支各占一列，条件出口使用独立节点；文本按实际字体宽度换行，节点高度随内容变化，不截断长标签。预览支持缩放和横向滚动，下载的 SVG 保持原始尺寸。

图中展示服务输入输出和步骤契约的顶层字段摘要（`?` 表示字段可选），引用按固定资源 ID、子服务实例 ID 解析，不取同名资源最新版。条件与路由展示返回值及固定 Python 入口函数原文；入口调用的辅助函数不展开，不将任意 Python 推断为等价自然语言条件。布尔出口、枚举字符串（包括字符串 `"true"`）、循环上限、分支返回来源和循环携带值分别展示。资源目录的只读 `entrySource` 字段来自已保存源码；升级桌面后需重启后端才能读取该字段。缺少资源或源码时明确显示不可用。

这是帮助理解执行顺序的只读视图，不是运行时 LangGraph 内部节点图，也不提供导入执行。输入输出为契约摘要，不是某次任务实际数据。图中省略参考数据连线、契约重试、预算和取消路径；子服务保留固定实例标识，不展开内部流程。草稿可预览，但不代表通过契约校验。旧协议仍通过原始快照查看，不自动转换。
...
if/while 的条件块选择器按严格 bool 出口筛选；switch 的路由块选择器按有限字符串枚举出口筛选，普通字符串、数字、对象和数组等明确不适用的块不会进入候选。无法静态解析的 Schema 保留到后端预检；已固定但不适用的引用仍显示并提示重新选择，不自动改接线。候选筛选不代表输入与参考兼容，保存时仍进行完整校验。repeat/foreach 的普通循环体不统一限制出口类型。
...
控制流交付通过离线边界、真实 Python 子进程、Electron 保存和隔离 PostgreSQL 重启验证，详见 [历史记录](archive/2026-09-21/control-flow-expansion-plan.md)。该验证没有调用外部模型，不覆盖具体业务内容质量。
