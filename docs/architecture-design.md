...
标准调用：严格校验输入及业务参数 → 读取快照 Prompt → 替换 {{input.primary.field}} / {{input.references[0]}} / {{parameters.field}} → 构造并校验 ModelRequest → 一次模型请求与计量 → 校验 ModelResponse → 严格校验业务输出。系统消息自动携带输出 JSON Schema，返回值为校验后的业务对象。错误不做自动修复或重试。
...
- 首期不配置持久化 checkpointer，不启用 LangGraph 通用自动重试、节点结果缓存或失败恢复；普通节点仅对输出契约失败按服务 retryLimit 有界重试；任务历史按所选仓储维护。PostgreSQL 保存记录与 LangGraph 断点恢复是两项独立设计，不自动同时开启。
...
- 期望输出由配置者选择已注册的契约资源，输入契约同理；可以复用模块 primaryContract/outputContract 业务契约，新增业务契约由开发者提供 Python 模型。首期不建设任意 Schema 设计器。最终输出绑定必须覆盖期望结构，并在保存及运行时分别校验。
...
当前采用模块化单体：资源目录、流程、运行时、适配器与文档仓储分工。保持单 API 进程与一个有限并发任务消费者池；内存队列、环境占用锁及快照对象尚不能直接跨进程共享，不通过增加 Uvicorn workers 扩容。
...
| R4 执行扩展 | 先测量队列等待、节点耗时与内存增长，单进程受控并发已交付，后续按测量结果设计背压、跨进程任务认领与租约 |
