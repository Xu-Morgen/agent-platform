...
更新日期：2026-09-21。本页保留当前运行结构和实现约束；产品范围与未完成边界见 [产品说明](product-requirements.md)。旧设计、查重算法和迁移记录见 [归档原文](archive/2026-09-18/architecture-design.md)。
...
2026-09-20 新增 `ServiceNode` 固定版本组合：保存与恢复解析固定叶子快照，父身份计入子实例摘要，执行时切换子流程上下文并保留父任务预算、环境占用、取消和图步数边界。内部节点记录使用服务路径，不整体重试子流程。共享契约选择身份从规范化 Schema 及可比较校验规则推导，不替换历史引用或任意自定义验证器。完整约束见[服务组合](service-composition.md)。
...
标准调用：严格校验输入及业务参数 → 读取快照 Prompt → 替换 {{input.primary.field}} / {{input.references[0]}} / {{parameters.field}} → 构造并校验 ModelRequest → 一次模型请求与计量 → 校验 ModelResponse → 严格校验业务输出。系统消息自动携带输出 JSON Schema，返回值为校验后的业务对象。模型适配器不自动修复输出或重试传输；流程执行器仅按下述契约失败规则重试节点。
...
角色由 Prompt 包的指令、参数和输入输出契约表达；数据通过 primary/references 在合法作用域内传递，Python 条件块决定已配置分支和有限循环。规划、审题等模型输出是业务数据，不是可执行流程；固定子服务也由父实例明确选择版本。现有机制足以组织当前出题场景的多角色协作，因此不增加独立 Agent 注册、私有记忆、主动通信、协商与生命周期调度层，避免形成另一套状态、预算和终止规则。

任务并发由消费者池控制，默认 4、范围 1～64，单任务内部仍按流程顺序执行；这一调度能力不等同于任务内多 Agent 协作。references 不跨任务保存记忆，也不能任意读取其他分支或循环轮次的残留结果。此处是本项目的架构选择，不以是否支持动态创建角色来判定其他系统是否属于多 Agent。

### 12.2 记录持久化不扩展为执行恢复

当前任务记录用于查询、审计和故障定位，未作为恢复协议保存和提交图执行检查点。可靠续跑不能通过读取“最后一个成功步骤”直接实现：还需要一致地恢复分支和循环位置、主数据与参考、节点/任务累计预算、取消状态及实例、依赖和环境绑定。若重做上游导致输出变化，依赖旧输出的下游结果不能直接复用。

外部 API 或模型请求可能已经完成，而本地还没有持久化成功状态；仅增加检查点不能消除重复副作用和重复计费风险。恢复还需定义操作幂等或结果核对、恢复触发、同一任务的排他执行和版本兼容规则；若扩展到多副本，则另需跨进程协调。这些都是新增恢复机制的设计成本，不是当前单后端已经具备的能力。

当前小任务不承担这些复杂度：不配置执行恢复 checkpointer、不新增 resume 接口，重启只恢复记录，遗留未完成任务标记 `APPLICATION_INTERRUPTED`。重新提交创建独立新任务，使用提交时的实例与环境，不能冒充续跑原任务。Java 或消息队列重投同样无法恢复平台内部执行位置；本项目不因此新增外部调度基础设施。

## 13. flow-6 入口与历史边界

NodeInput 的 Python 固定元组在 JSON 中为数组，仅转换 references 容器，不关闭严格元素或自定义校验。节点保存已校验主输入副本和输出，子分支及循环每轮使用独立状态，不能通过下游原地修改历史值。Prompt 只发送占位符明确使用的路径，不隐式附带参考。API 仍独立校验业务请求与响应。
