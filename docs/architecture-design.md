...
主数据字段错误在直接生产节点的下游入口预检中归因并重试；参考错误与完整入口跨字段错误立即失败。容器和历史节点不整体重放，条件输出错误只重试条件块。编译器为 flow-5；历史协议文档按原字节保留、不进入新 FlowDraft 校验与编译，允许查看历史和任务终态，但禁止执行、回退激活及直接复制为新协议草稿。升级资源并在服务页重新保存会产生新版本。
