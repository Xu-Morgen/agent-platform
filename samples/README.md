...
当前执行协议为 flow-6，块与最小包为 3.0.0，完整包为 4.0.0；实际版本以装饰器和 package.json 为准。独立契约按源码摘要区分。NodeInput、参考槽位和控制容器的接线规则集中在 [USAGE.md](USAGE.md)；资源导入、版本冲突、最新版本展示与归档规则见 [资源手册](../docs/resource-guide.md#更新与归档资源)。

修改说明也会改变资源摘要，应递增版本并重新导入；旧实例不自动升级。资源用途和字段说明见 [AI 解释手册](../docs/resource-explanation.md)，foreach/switch 教学源码见 [控制流手册](../docs/control-flow.md)。原查重产品及旧导入脚本已移除，不属于当前模板。
