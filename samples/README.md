...
当前执行协议为 flow-5，块/包模板均为 3.0.0。主数据与参考由平台统一封装为 NodeInput；模块目录的 primaryContract 是可选服务入口，inputContract 是完整节点入口。完整包演示固定单参考，完整独立契约演示 ReviewInput；不增加第三套示例。旧快照可查历史，升级资源后须重新保存实例。

## 质量流程与模板版本

当前通用模板仍为 flow-5 的 `NodeInput` 资源，包/块版本为 3.0.0；文档出题业务的版本独立管理，出题包现为 5.0.0，不意味着所有通用模板都必须同号升级。不要只改版本号而不检查源码内容和快照冲突。
