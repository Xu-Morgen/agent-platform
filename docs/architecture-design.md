...
- 每个业务模块有一个契约化输入值和输出值，页面展示对象字段与约束，但不能选取或拼装字段。普通步骤不配置输入绑定，输入输出契约由源码或清单固定声明；第一步接收服务完整输入，其余步骤只接收上一层完整输出。分支内第一步接收进入分支前的完整数据；循环体及条件通用块的第一步接收本轮完整携带值。服务返回仅配置输出契约，自动校验并返回顶层最后一步的完整输出；最后一步为分支或循环时返回该步骤的完整输出，空流程校验失败。
