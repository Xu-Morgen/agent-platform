...
通用块的[最完整示例](blocks/README.md#最完整实现)包含 API 查询、严格请求与响应校验、可选附件、进度、明确错误及文本处理；运行前在块节点配置 API 连接和路径。模板之间的结构转换及参考配置统一见 [接线说明](USAGE.md#模板之间的接线)。
...
当前执行协议为 flow-6，最小块与最小包为 3.0.0，完整块为 3.0.1，完整包为 4.0.2；实际版本以装饰器和 package.json 为准。独立契约按源码摘要区分。NodeInput、参考槽位和控制容器的接线操作集中在 [USAGE.md](USAGE.md)；资源导入、版本冲突、最新版本展示与归档规则见 [资源手册](../docs/resource-guide.md#更新与归档资源)。

修改说明也会改变资源摘要，应递增版本并重新导入；旧实例不自动升级。资源用途和字段说明见 [AI 解释手册](../docs/resource-explanation.md)，foreach/switch 教学源码见 [控制流手册](../docs/control-flow.md)。
