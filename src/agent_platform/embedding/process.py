...
class EmbeddingProcesses:
    ...
    def session(self, root, snapshot):
        if self.closed:
            raise failure('EMBEDDING_NOT_READY', '模型进程管理器已关闭')
...
class Session:
    ...
    async def start(self):
        ...
        try:
            ...
            raise failure('EMBEDDING_CAPACITY_EXCEEDED', '本地模型工作进程已占用，等待超过 120 秒') from None
        self.acquired = True
        self.queue_seconds = perf_counter() - started
        if self.closed or self.owner.closed:
            await self.close()
            raise failure('EMBEDDING_NOT_READY', '本地模型会话已关闭')
        ...
        self.process = await asyncio.create_subprocess_exec(sys.executable, '-B', '-m', 'agent_platform.embedding.worker',
    ...
    async def exchange(self, message):
        try:
            ...
            raise failure('EMBEDDING_INFERENCE_ERROR', '本地模型工作进程退出、超时或响应无效') from None

    async def call(self, operation, **values):
        async with self.lock:
            if self.closed:
                raise failure('EMBEDDING_NOT_READY', '任务模型上下文已清理')
            try:
