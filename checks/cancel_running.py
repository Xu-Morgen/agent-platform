...
async def case(mode='success'):
    ...
    async with local_http(responder) as url:
        ...
        env = app.state.environments.save(EnvironmentWrite(name='取消替身', connections=[{
            'connectionId':'model','kind':'model','baseUrl':url,'model':'synthetic','timeoutSeconds':3}]))
        ...
        async with app.router.lifespan_context(app):
            ...
            async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
                ...
                assert result.status == 'cancelled' and result.cancel_requested and result.cancel_phase is None
                assert result.result is None and len(calls) == 1 and later_calls == []
                assert result.usage['tokens']['global']['totalTokens'] == 5
                assert not app.state.environments.get(env.environment_id).active_run_ids
                assert (await client.post(f'/api/v1/runs/{run.run_id}/cancel')).json()['status'] == 'cancelled'
    print('PASS: 本地模型等待时取消仍 running/waiting_transport；响应后 cancelled；已用 token 保留；环境释放')
