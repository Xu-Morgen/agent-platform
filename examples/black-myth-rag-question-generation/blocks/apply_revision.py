...
@block(id='rag-question-apply-revision', version='3.0.0', name='采纳修订并重新待审', description='主数据 Questions；参考 tuple[State]；输出 State。')
def run(value: NodeInput[Questions, tuple[State]]) -> State:
    try:
        ...
        check_coverage(value.primary.questions, old.planned.normalized.items)
