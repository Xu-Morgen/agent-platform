...
- 查重输出确定性定量报告和模型定性报告，供人工判断；不生成最终抄袭判定。
...
| `samples/template/` | 可复制的三类资源与控制流配方 |
| `samples/assignment-similarity/` | 查重算法权威源、分发块、语义包和配方 |
| `samples/flow_templates.py` | 加载资源并导入会话草稿 |
| `samples/invoke_similarity.py` | 使用业务输入调用查重服务 |
| `scripts/export_contracts.py` | 从权威模型生成桌面控制协议 Schema |
| `docs/` | 当前需求、架构、计划、手册和历史交接 |
| `artifact/` | 用户保留资料，本轮未改动 |

旧测试脚本、测试数据和旧实例执行入口已移除。历史交接中的旧命令仅作记录，当前入口以本页和手册为准。开发约定见 [AGENT.md](AGENT.md)。
