...
I3 验收使用合成数据、本地 HTTP 模型协议替身及真实 Electron 页面，尚未验收真实模型。预算执行、主动取消、完整在途退出控制和查重产品留 I4—I5；当前不要将配置中的预算视为已生效限制。首个适配器不声明严格总 token 保证，详见 [模型协议能力](docs/protocols/model.md)。PostgreSQL 仅为后续选型，当前未启用持久化。
...
排队、运行或取消等待任务占用的环境不可修改。失败保留任务终态并清理运行上下文。以下控制语义仍待 I4：主动取消等待、取消期间错误优先级、桌面控制管道立即中止在途传输。

I4 将接入包调用 loop 计数与可配置 token 预算。I3 已有尝试记录与策略注入入口，但尚未装配预算策略；不自动重试、不实现去重键。
