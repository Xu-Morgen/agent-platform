...
```json
{
  ...
  "limits": {"topK": 30, "contextCharacters": 20000}
}
```
...
正式服务“知识库三题出题 · ds”（`svc_303e8991d0124b4eb92de3fbc0b4edc3`）与“黑神话材料阅读出题”知识库已保存到桌面数据目录。请求示例推荐选取 30 个片段、最多 20,000 字符；默认 8 个片段可以使用，但可能无法覆盖三题的证据需求。
...
最终默认实例为 4.0（`ins_dc1fb98ee71f4e01bf44505fed955cc8`）。20 次 ds 调用已验证默认/替换检索主路径、不支持请求及两种资料不足；原件、知识库、服务和任务历史跨重启保留。人工评阅尚待用户确认，未标记人工通过。可在任务页打开 `run_8020c33ecdfd41bfaba96d72ac75c12a` 核对三题和来源。
