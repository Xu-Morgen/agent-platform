...
Electron 主进程启动并管理一个 Python 后端子进程。后端运行单个 Uvicorn worker 和 asyncio 事件循环，承载 FastAPI、仓储、任务队列、LangGraph 执行及异步 I/O。任务队列由可配置的并发消费者池领取，默认 4 个消费者，范围 1～64；配置入口见 [任务并发配置](../readme.md#任务并发配置)。每个消费者等待当前任务及清理完成后才领取下一条，单个任务内部仍按流程顺序执行。后端通过父子进程控制管道通知实际监听地址；健康检查就绪后页面才开放操作，启动失败显示原因。
...
Python 通用块每次调用创建独立子进程；有依赖的块使用锁定的独立 venv，无额外依赖的块使用 SDK Python。单次业务调用上限 300 秒，CPU 计算取消会终止子进程；API 请求由父进程转发，凭据不发给子进程，主动取消仍等待在途传输。详见 [运行文件与依赖](platform-runtime-files.md)。

HTTP 默认监听 `127.0.0.1`，桌面显示实际地址和端口；需要局域网调用时通过启动配置指定监听地址。首期不增加权限服务。桌面退出时停止 HTTP 服务，不保留后台常驻进程；禁止多个 Uvicorn 后端进程共享存储，避免内存记录分裂；单后端内允许多个异步任务消费者。
...
- 按明确产品边界，不配置用于断点续跑的持久化 checkpointer，不启用 LangGraph 通用自动重试、节点结果缓存或失败恢复；普通节点仅对输出契约失败按服务 retryLimit 有界重试；任务历史按所选仓储维护。PostgreSQL 保存记录与 LangGraph 断点恢复是两项独立设计，不自动同时开启。
...
- 每个业务模块有一个契约化输入值和输出值，页面展示对象字段与约束，但不能选取或拼装字段。普通步骤主数据不配置来源，输入输出契约由源码或清单固定声明；第一步 primary 接收服务完整输入，其余步骤的 primary 接收上一层完整输出。简单模式参考上一节点此次的 primary，服务第一步零参考；高级列表替换默认参考，按固定元组位置严格校验。if 条件及分支首节点沿用进入 if 的主数据与默认参考；循环条件和循环体首节点以当前 carry 为主数据、默认零参考。容器后的默认参考是容器入口主数据，循环为初始 carry。服务返回仅配置输出契约，自动校验并返回顶层最后一步的完整输出；最后一步为分支或循环时返回该步骤的完整输出，空流程校验失败。
...
- 期望输出由配置者选择已注册的契约资源，输入契约同理；可以复用模块 primaryContract/outputContract 业务契约，新增业务契约由开发者提供 Python 模型。首期不建设任意 Schema 设计器。顶层最后一步的完整输出必须满足期望结构，并在保存及运行时分别校验；没有最终输出来源选择。
...
点击包节点打开悬浮窗，依配置 Schema 显示字段、说明、默认值、必填与校验错误；嵌套参数仍可通过 JSON 编辑区填写，尚未提供完整嵌套表单；选择已保存环境中的模型连接。保存节点配置后重新校验节点，主流程保持草稿；保存服务版本另走全流程校验。通用块显示函数说明、输入输出和连线；声明 api=True 的块提供 API 连接选择和必填的请求路径输入框，无模型或预算选项。路径没有默认值，预检拒绝空路径、完整 URL、查询字符串、片段和目录回退。
...
资源源码按内容摘要固定。包与契约从保存的内容加载；块在对应环境的子进程中导入固定源码，不依赖跨调用全局变量，不因导入新版本改写旧实例执行内容。平台标准入口读取包 Prompt 的内存快照，首期模板不依赖源文件绝对路径。该加载方式属于受信任代码管理，不是沙箱。
...
包的业务参数 Schema 不包含平台预算。节点 budget 暴露正整数 `loopLimit`、`tokenLimit`，包可选 budgetDefaults 仅提供初始化默认值，未声明时使用平台默认 4/32768。节点显式配置覆盖包默认值，保存后固定为节点在单任务内允许累计使用的上限。含包实例配置另包含单任务全局 `loopLimit`、`tokenLimit`，界面展示局部与全局约束。
...
通用块已有进程执行边界，但没有第三方代码权限沙箱。当前未提供队列容量、背压、按连接限流、跨进程任务租约。多 Agent 框架与任务断点续跑明确不考虑，不作为扩展待办；LangGraph 不配置执行恢复检查点。存储恢复只恢复记录，不重放外部操作。备份工具、历史和运行环境的引用安全清理等未完成事项见 [产品说明](product-requirements.md#7-未实现能力与验收边界)。

### 12.1 用预定义多角色流程满足当前需求

角色由 Prompt 包及固定契约表达，通过 primary/references 传递任务内数据，Python 条件块选择预定义分支和有限循环。固定子服务明确绑定实例，任务并发由消费者池控制；不增加 Agent 私有记忆、通信或协调层。产品取舍与理由统一见 [产品决策](product-requirements.md#明确不考虑的能力2026-09-21)。

### 12.2 记录持久化不扩展为执行恢复

任务记录用于查询、审计和定位，未按恢复协议原子提交执行位置、分支/循环、主数据/参考、预算及外部副作用状态。当前不配置执行恢复 checkpointer，也没有 resume 接口；重启只恢复记录，将遗留未完成任务标记 `APPLICATION_INTERRUPTED`。重新提交创建独立任务，使用提交时的实例与环境；外部队列重投也不能恢复平台内部执行位置。产品理由见 [产品决策](product-requirements.md#明确不考虑的能力2026-09-21)。

## 13. flow-6 入口与历史边界

NodeInput 的 Python 固定元组在 JSON 中为数组，仅转换 references 容器，不关闭严格元素或自定义校验。节点保存已校验主输入副本和输出，子分支及循环每轮使用独立状态，不能通过下游原地修改历史值。Prompt 只发送占位符明确使用的路径，不隐式附带参考。API 请求与响应的校验职责见 [单一契约来源](#54-单一契约来源)，不能把 NodeInput 校验等同于外发 payload 校验。

主数据字段错误在直接生产节点的下游入口预检中归因并重试；参考错误与完整入口跨字段错误立即失败。容器和历史节点不整体重放，条件输出错误只重试条件块。编译器为 flow-6；flow-5 及更早历史协议文档按原字节保留、不进入新 FlowDraft 校验与编译，允许查看历史和任务终态，但禁止执行、回退激活及直接复制为新协议草稿。升级资源并在服务页重新保存会产生新版本。


## 14. 数组遍历与枚举分支

foreach 显式选择完整来源及确定对象字段路径，静态推导元素入口，元素出口契约派生 `{items: list[元素]}` 集合 Schema。运行时先固定数组并检查最大条数，再顺序执行独立元素作用域；体首节点默认零参考，当前元素与 carry 分开保存。每项出口严格校验，任一项失败则整个任务失败，不公开部分成功集合。下游默认参考仍为容器入口完整主数据。

switch 路由为 NodeInput Python 块，输出有限字符串 Literal。静态检查所有出口完整且唯一，分支输出统一校验；路由值不替换主数据，分支内部结果不泄露到外层。if/while 继续严格 bool。两类容器均沿用固定快照、任务预算、取消及图步数检查；不重放容器以重试集合或历史参考错误。

节点遍历统一覆盖路由、枚举分支和数组体，保存/恢复收集全部资源与出口契约。flow-6 参与快照摘要，新版本不改写旧快照或固定子服务引用。操作和字段说明见[控制流手册](control-flow.md)，实际验证见[归档计划](archive/2026-09-21/control-flow-expansion-plan.md)。
