...
from ..contracts.ocr import OCRModel, OCRSelection, OCRSnapshot
...
class OCRRepository:
    def __init__(self, store, processes, root=None):
        ...
        if root is None:
            if store.durable:
                ...
                root = Path(os.environ.get('AGENT_PLATFORM_DATA_DIR', default_data_directory())) / 'ocr-models'
            else:
                self.temporary = TemporaryDirectory(prefix='agent-ocr-')
                root = self.temporary.name
        self.root = Path(root)
        self.root.mkdir(parents=True, exist_ok=True)
        self.models = {key: OCRModel.model_validate(value) for key, value in store.read('ocr_models').items()}
        self.selection = OCRSelection.model_validate(store.read('settings').get('ocr', {}))
        # 启动恢复核对持久文件，历史身份不随文件损坏改变。
        for key, model in self.models.items():
            try:
                if read_manifest(self.root / key) != model.manifest:
                    raise failure('OCR_NOT_READY', '应用模型清单与持久记录不一致')
                verify_files(self.root / key, model.manifest)
            except Exception as exc:
                model.status = 'unavailable'
                model.error = getattr(getattr(exc, 'error', None), 'message', '应用模型文件不可读取')
                store.write([('ocr_models', key, model.model_dump(mode='json'))])
    ...
    def get(self, model_id):
        with self.lock:
            if model_id not in self.models:
                raise failure('OCR_NOT_READY', '模型尚未导入，请在平台设置中导入本地模型目录')
            return self.models[model_id].model_copy(deep=True)

    def fixed(self):
        with self.lock:
            if self.selection.model_id is None:
                raise failure('OCR_NOT_SELECTED', '请在平台设置中选择就绪的本地 OCR 模型')
            model = self.get(self.selection.model_id)
            if model.status != 'ready':
                raise failure('OCR_NOT_READY', '默认模型不可用，请恢复应用模型文件或导入另一版本')
            return OCRSnapshot(model_id=model.model_id, manifest=model.manifest)
    ...
    def select(self, value):
        with self.lock:
            if value.model_id is not None:
                ...
                if model.status != 'ready':
                    raise failure('OCR_NOT_READY', '所选模型未就绪')
            self.store.write([('settings', 'ocr', value.model_dump(mode='json'))])
    ...
    async def check(self, model_id):
        ...
        session = self.processes.session(self.root / model_id,
            OCRSnapshot(model_id=model_id, manifest=model.manifest))
        try:
            ...
            await session.close()
            with self.lock:
                self.store.write([('ocr_models', model_id, model.model_dump(mode='json'))])
        ...
        async with self.import_lock:
            ...
            with TemporaryDirectory(prefix='.import-', dir=self.root) as temporary:
                ...
                def copy():
                    ...
                    (target / 'ocr.json').write_text(manifest.model_dump_json(by_alias=True, indent=2))
                ...
                try:
                    ...
                    await copying
                    raise
                progress('验证 CPU 模型加载与小样例识别')
                snapshot = OCRSnapshot(model_id=model_id, manifest=manifest)
                session = self.processes.session(target, snapshot)
                try:
                    await session.call('diagnostic')
                finally:
                    await session.close()
                model = OCRModel(**snapshot.model_dump(), size_bytes=size,
                ...
                with self.lock:
                    ...
                    if destination.exists():
                        ...
                        if read_manifest(destination) != manifest:
                            raise failure('OCR_NOT_READY', '已存在的模型目录身份冲突')
