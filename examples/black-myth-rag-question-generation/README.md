...
2026-09-22 数量升级后全部业务包和业务块为 3.0.0。模型 Prompt 只渲染选用证据，不渲染全部候选；候选仍留在任务记录中。首次 1.0 实验发现的越界引用已被严格核验阻止，失败历史保留，未改写成成功。最终任务页独立显示原始业务状态，不把 `insufficient_source` 等业务结论当成合格题目。
...
正式服务已通过页面升级为 5.2（`ins_9861394135c34605b66f83a31332fb53`），全部业务节点固定 3.0.0 资源，分支出口和循环携带值使用新版契约。当前 foreach 最大条数为 1,000，任务调用预算为 1,007，单个生成节点调用预算为 1,000；任务和节点 token 预算均为 10,000,000，模型单次输出上限为 8,192。以上是页面可调运行参数，不是业务题数上限。真实验收使用固定的 5.0 快照，任务调用预算为 10，防止超过用户本次累计 30 次授权。
