...
@block(id='rag-question-merge-review', version='4.0.0', name='登记题目审查历史', description='主数据 Review；参考 tuple[State]；输出 State。')
def run(value: NodeInput[Review, tuple[State]]) -> State:
    try:
        ...
        check_review(value.primary, old.planned.context, old.planned.normalized.items)
