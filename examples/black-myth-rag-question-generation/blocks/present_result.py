"""将经过校验的内部出题结论转换为对外题目结果；内部资料保留在任务步骤记录。"""
...
class DisplayChoice(StrictModel):
    question_id: Annotated[str, Field(pattern=r'^choice-[1-9][0-9]*$')]
    type: Literal['single_choice']
    stem: Text
...
class DisplayJudgment(StrictModel):
    question_id: Annotated[str, Field(pattern=r'^judgment-[1-9][0-9]*$')]
    type: Literal['true_false']
    stem: Text
    answer: bool
...
class DisplayEssay(StrictModel):
    question_id: Annotated[str, Field(pattern=r'^essay-[1-9][0-9]*$')]
    type: Literal['essay']
    stem: Text
    answer: Text
...
class QuestionDelivery(StrictModel):
    status: Literal['completed']
    questions: list[DisplayChoice | DisplayJudgment | DisplayEssay] = Field(min_length=1)


class UnavailableDelivery(StrictModel):
    status: Literal['insufficient_source', 'unsupported_request', 'quality_not_met']
    reason: Text


class PublicResult(StrictModel):
    """对外结果权威契约；不暴露 State、知识库、检索或内部引用字段。"""
    result: QuestionDelivery | UnavailableDelivery


@block(id='rag-question-present-result', version='1.0.0', name='整理出题交付结果',
       description='主数据 Result；参考为空。核对最终审题结论后，仅交付题目、选项、答案、解析和得分点；未通过时返回状态与原因。知识库、检索证据与审题过程保留在任务历史。输出 PublicResult，放在最外层请求分支之后作为服务最后一步。')
def run(value: NodeInput[Result, tuple[()]]) -> PublicResult:
    try:
        result = check_result(value.primary).result
        if isinstance(result, Completed):
            questions = []
            for question in result.state.current.questions:
                display = (DisplayChoice if isinstance(question, Choice) else
                           DisplayJudgment if isinstance(question, Judgment) else DisplayEssay)
                # 按公开契约白名单取值，内部模型以后新增字段也不会被意外带出。
                fields = {name: getattr(question, name) for name in display.model_fields}
                questions.append(display.model_validate(fields))
            return PublicResult(result=QuestionDelivery(status='completed', questions=questions))
        return PublicResult(result=UnavailableDelivery(status=result.status, reason=result.reason))
