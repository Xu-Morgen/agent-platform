...
[文档管理与最小 RAG 平台基础](docs/knowledge.md)及独立的[知识库出题业务](examples/black-myth-rag-question-generation/README.md)已交付。正式服务“知识库出题 · ds”仅支持单选、判断、论述，数量可自由组合，支持逐题多文档证据；旧固定三题版本的 20 次授权真实调用通过默认/替换检索主路径、不支持请求和资料不足检查。用户于 2026-09-22 确认旧版人工验收通过；数量升级新增 12 次真实调用验证“2 道单选与 2 道判断”和“仅 2 道论述”（累计 32/100 次），随后用户发现论述内容混入平台溯源要求，已升级正式服务至 6.0；历史模型 pass 不代表内容质量验收，修复边界见手册。真实修订/重出等未触发路径见手册，旧全文出题实例不自动升级。
