...
这些是可替换的普通资源，长期放在 `resources/rag/`，不占用 samples 的最小/完整模板位置。PDF 读取块声明固定解析/OCR 依赖与模型，其余块使用平台 SDK 与 Python 标准库；块本身不加载向量依赖、无持久化索引；可选语义块调用平台管理的 CPU 模型。共享数据契约由 SDK 的 `contracts/retrieval.py` 定义，独立入口为 `contracts.py`；平台编译器不引用该检索契约，也不识别专用 RAG 节点。
