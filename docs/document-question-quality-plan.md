...
日期：2026-09-20。状态：需求与方向已确认，扩展实现和真实模型验收均未开始。本次仅编写计划。

已完成前置依赖：[节点输入机制与 Python 条件块计划](archive/2026-09-20/node-input-and-python-conditions-plan.md) P1–P4 已交付并完成轻量验收；本计划尚未开始实施。当前已交付的读取与一次出题用法、OCR 限制和未完成验收继续见[实例说明](../examples/document-question-generation/README.md)。本计划不替代旧交付记录，也不代表已上线审题修订。
...
图为目标设计，资料不足、校验失败、取消和预算耗尽等错误边不逐一绘出，均明确终止。
...
全部步骤当前均未开始；可先明确业务契约设计，但运行实现依赖平台前置目标完成。
...
- [ ] 原文、规划和当前题目在多轮中保持可用，模型输出不能覆盖原文。
- [ ] 首次 pass 不修订；revise 与 regenerate 分别走正确分支；每轮基于最新完整状态。
- [ ] 持续不合格达到上限明确失败；资料不足、非法 verdict、错误引用与契约失败均不能伪装成功。
- [ ] 现有三题顺序、单选选项、填空和非空校验继续生效；引用存在性与语义支持分别检查。
- [ ] 修订、审题、技术重试计数正确；预算、取消和应用退出的任务终态及记录正确。
- [ ] 真实文档比较当前一次出题基线与新流程：使用相同文档和评分标准，记录发现的问题、是否修好、是否引入新错误、实际调用和 token，不预先承诺提升比例。
- [ ] 审题可独立绑定模型连接；同一模型的独立调用可作为首版配置，但模型互审不能替代人工核验。
- [ ] 本地 OCR 未完成项继续保留，不能把读取成功等同于复杂排版或业务质量验收通过。

此前用户要求暂不调用线上模型仍有效。本次只授权写计划，不执行付费调用；Q4 标为待用户解除该限制并指定可用连接后执行。在此之前可以完成资源和离线协议验证，但不得将整个业务质量目标标为验收通过。
