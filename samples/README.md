...
通用块的[最完整示例](blocks/README.md#最完整实现)包含 API 查询与文本处理；API 连接和请求路径配置在块节点上。最小文本块的输出可直接接入最小包。完整块需选取输出中的 `text` 字段接入最小包，因为其输出还包含统计字段。完整包要求 `text` 为 1～10000 字符，现有块的输出没有这一保证，需先通过声明并验证该约束的转换块。完整独立契约演示批次结构，需配套处理节点，不能直接替换文本示例的端口。
